...
class Product
{
    ...
    {
        try {
            $query = "SELECT CODE, NAME, THUONGHIEU, DESCRIPTION,PRICE, SALEOFF,IMG1,IMG2,IMG3,IMG4, GROUP_CONCAT(distinct(SIZE)) AS SIZE, GROUP_CONCAT(distinct(COLOR) SEPARATOR '/') AS COLOR 
            FROM  ecommerce.product 
            WHERE CODE = '$id'
            GROUP BY CODE, NAME, THUONGHIEU, DESCRIPTION,PRICE, SALEOFF,IMG1,IMG2,IMG3,IMG4;";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
            return $stmt->get_result();
        } catch (mysqli_sql_exception $e) {
        ...
        try {
            ...
            foreach ($sizes as $size) {
                foreach ($colors as $x => $val) {
                    $query = "INSERT INTO product VALUES ('$CODE','$NAME','$val','$size','$THUONGHIEU','$DESCRIPTION',0,'$SALE','$PRICE','$IMG[0]','$IMG[1]','$IMG[2]','$IMG[3]','$CATEGORY')";
                    $stmt = $this->conn->prepare($query);
                    $stmt->execute();
                }
            }
    ...
    {
        try {
            $query = "SELECT DISTINCT(CODE), product.NAME, THUONGHIEU, DESCRIPTION, SALEOFF, PRICE, IMG1, IMG2, IMG3, IMG4, categories.NAME AS CATEGORY
    ...
    public function edit($data)
    {
        try {
            ...
            $DESCRIPTION = $data['DESCRIPTION'];
            $IMG = $data['IMG'];
            $query = "UPDATE product SET NAME='$NAME', PRICE='$PRICE',SALEOFF='$SALE',THUONGHIEU='$THUONGHIEU',
